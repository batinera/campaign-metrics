...
export class FacebookService {
  ...
  async getCampaigns(accountId?: string): Promise<Campaign[]> {
    ...
    const { data } = await this.client.get<{ data: Campaign[] }>(`/${actId}/campaigns`, {
      params: {
  ...
  async getInsights(dto: InsightsQueryDto): Promise<InsightRow[]> {
    ...
    const target = isCampaignQuery ? `/${dto.campaignId}/insights` : `/${actId}/insights`
  ...
  async getInsightsRaw(dto: InsightsQueryDto): Promise<{
    ...
    const target = dto.campaignId ? `/${dto.campaignId}/insights` : `/${actId}/insights`
  ...
  private sumAction(actions: ActionItem[] | undefined, type: string): number {
    ...
    const item = actions.find((a) => a.action_type?.toLowerCase() === type.toLowerCase())
  ...
  private aggregateInsightRows(rows: InsightRow[]): InsightRow {
    ...
    const actions: ActionItem[] = Object.entries(actionsByType).map(([action_type, value]) => ({
      action_type,
      value: String(value),
    }))
    ...
    return {
      ...
      cost_per_action_type: cost_per_action_type.length ? cost_per_action_type : undefined,
    }
  }

  toCampaignMetrics(rows: InsightRow[]): CampaignMetrics {
    const row = rows.length > 1 ? this.aggregateInsightRows(rows) : (rows[0] ?? {})
